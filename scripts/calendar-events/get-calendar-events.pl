...
my $GOOGLE_CALENDAR_ID = $ENV{GOOGLE_CALENDAR_ID}
    || die 'GOOGLE_CALENDAR_ID environment not set';
...
sub refresh_token {
    ...
    if ($res->is_success)  {
        ...
        if ($res->code eq '200') {
            ...
            say sprintf "refresh calendar api token OK. access token=[...%s], expires_in=%d", substr($access_token, -4), $res->json('/expires_in');
        } else {
            say sprintf "refresh calendar api token failed, http code=%s", $res->code;
        }

    }
    elsif ($res->is_error) {
        warn 'get token failed, error message: ' . $res->message;
    }
    else {
        warn 'get token failed, Unknown Error';
    }
...
sub set_reminder {
    ...
    my $rpc_result = $ua->post($wmatrix_url
        => json => {
            id => $event->{id},
            method => 'SetReminder',
            params => {
...
if (refresh_token()) {
    ...
    if ($res->is_success)  {
        ...
        if ($res->code eq '200') {
            ...
            my $start_time = (exists $next_event->{start}{date})
